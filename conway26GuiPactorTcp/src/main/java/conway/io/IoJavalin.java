...
//        }); 
        ...
        app.get("/", ctx -> {
            String pageToLoad;
            if (firstCaller == null) {
                // Primo utente: carica la pagina con i pulsanti
                pageToLoad = "/page/LifeIInOutCanvas.html";
                CommUtils.outmagenta("IoJavalin | Consegno pagina OWNER");
            } else {
                // Utenti successivi: carica la pagina senza pulsanti
                pageToLoad = "/page/LifeNoButtons.html";
                CommUtils.outmagenta("IoJavalin | Consegno pagina OBSERVER");
            }

            var inputStream = getClass().getResourceAsStream(pageToLoad);
            if (inputStream != null) {
                String content = new String(inputStream.readAllBytes(), StandardCharsets.UTF_8);
                ctx.html(content);
            } else {
                ctx.status(404).result("Errore: file " + pageToLoad + " non trovato");
            }
        });
